---
title: "Supplementary Methods Wetlab: Automated Sample Preparation and Wet Lab Protocols for Integrated PTM Analysis"
...
2. **Nuclease (optional):** 5 U/$\mu L$ Benzonase Nuclease (Merck, Germany, Cat. #70664-3; store at -20°C, stable for 2 years)
...
8. **Pipetting:** Multichannel pipette (8 or 12 channel, 10-300 $\mu L$ range)
...
1. **UHPLC system:** Vanquish Flex UHPLC with fraction collector and 100 $\mu L$ sample loop (Thermo, Cat. #VF-S01-A; includes autosampler, pump, column compartment, and fraction collector)
2. **Software:** Chromeleon 7.3.1 instrument control software (Thermo, included with system)
3. **Standard:** BSA peptide standard: 500 μmol dry, diluted in 600 $\mu L$ water to 83.3 μmol final concentration for 100 $\mu L$ injection (Sigma-Aldrich, Cat. #A7906)
...
1. Add up to 50 $\mu L$ lysis buffer for small pellets (≤10 $\mu L$ pellet volume) and more for larger pellets, using 3×–5× the pellet volume and completely resuspend and disrupt cells in buffer by pipetting up and down; apply 1 min Tissue Lyzer (30 Hz) cycles with glass beads if cell pellet is not completely disrupted and lysed (**Critical:** Complete cell disruption is essential for protein extraction efficiency)
2. For tissue samples, add 300 $\mu L$ lysis buffer to up to 5 mg of tissue (optimal volume depends on tissue origin and protein content) and disrupt the sample using the Tissue Lyzer II with 2× 1 min cycles (30 Hz) (**Time:** 5 min total homogenization)
...
9. Snap-freeze samples in liquid nitrogen and store at -20°C or proceed with the protocol for 30–50 $\mu g$ of total protein with 0.5–2 $\mu g$/$\mu L$ (**Stopping point:** Samples can be stored for up to 6 months)
...
13. Prepare carboxylated magnetic beads (see Note 4.2)
    ...
    2. Beads are available at a stock concentration of 50 μg/$\mu L$. Take the required volume of beads and mix hydrophilic and hydrophobic beads at a 1:1 ratio
    3. Wash beads three times with water and add the respective amount in 250 $\mu L$ to each well
...
15. Fill three deep-well plates with 500 $\mu L$ wash buffer (80$\%$ EtOH) (**Preparation time:** 10 min)

16. Add 100 $\mu L$ of trypsin in 20 mM TEAB at an enzyme:protein ratio of 1:50 to a microwell plate (**Critical:** Use MS-grade trypsin)
...
17. Process samples with KingFisher protocol as described in Leutert et al. with minor changes (**Time:** 2 hours automated processing)
    ...
    2. The next day, return the digestion plate to its original KingFisher slot, add 100 $\mu L$ water to a second microwell plate, and place it in the empty KingFisher slot
...
18. Resuspend peptides in 45 $\mu L$ 50 mM TEAB and sonicate in a water bath for 10 min (**Expected recovery:** >90$\%$ of starting peptides)

19. Resuspend 100-200 $\mu g$ TMTpro reagent in 15 $\mu L$ anhydrous acetonitrile, vortex, and spin down (see Note 4.4) (**Critical:** Use immediately after resuspension)
...
21. Add 3.5 $\mu L$ of 5$\%$ hydroxylamine to quench the reaction (**Time:** Immediate quenching essential)
22. Pool a small fraction of each sample (e.g.,1 $\mu L$, ensure precise pipetting) and pool at equal ratio to check for sample loading variation and labeling efficiency using LC-MS/MS in QC step (**Critical QC step:** Assess before proceeding; see Note 4.11)
...
1. For robust and sensitive performance of phosphoproteomics experiments, we typically aim for 10-25 $\mu g$ of peptide per fraction. These amounts have been demonstrated to be effective for automated protocols on the KingFisher Flex ([10.1016/j.mcpro.2024.100754](https://doi.org/10.1016/j.mcpro.2024.100754), [10.1002/pmic.202100245](https://doi.org/10.1002/pmic.202100245)). We generate 36 fractions, corresponding to a total of approximately 180-540 $\mu g$ of pooled peptide before fractionation
2. Resuspend the peptides in 100 $\mu L$ of high-pH buffer A, and sonicate for 5 min to ensure complete resuspension. Pellet insoluble material and only transfer supernatant into the HPLC vial
...
### 3.3.2 Sample splitting and workflow assignment

After TMT labeling and pooling, split the sample into two workflows:

- **Aliquot A (5-10$\%$ of total):** Total proteome analysis (proceed to section 3.3.6)
- **Aliquot B (90-95$\%$ of total):** Phospho-enrichment analysis (proceed to section 3.3.3)


### 3.3.3 Phosphopeptide enrichment and Evotip loading

1. Resuspend each fraction in 150 $\mu L$ Ti-IMAC buffer I (see Note 4.9), sonicate for 10 min
...
4. Wash the beads with 200 $\mu L$ Ti-IMAC buffer I, collecting the beads on a magnetic rack each time and discarding the wash buffer. Repeat for a total of three washes, then add beads to the microwell plate in a final volume of 150 $\mu L$ per well
5. Prepare three additional microwell plates, each with 150 $\mu L$ of Ti-IMAC buffer I, II, and III
...
7. Add 80 $\mu L$ Ti-IMAC elution buffer to each well of a microwell plate and add the plate to the indicated KingFisher slot during the pause after the last wash step, and continue with the peptide elution
8. Immediately after elution, neutralize peptides by adding 10 $\mu L$ of neutralization buffer
9. Follow the Evotip pure loading protocol for peptide binding to Evotips

### 3.3.4 Optional antibody-based enrichment
...
1. Resuspend one half of the pooled labeled peptides in 400 $\mu L$ cold IP binding buffer, vortex, and incubate rotating at 4°C for 5 min, check pH (should be neutral or slightly basic, not below pH 7)
2. Spin down solution 5 min at 20,000×g to clear the solution and pellet insoluble material
3. Gently mix the antibody bead slurry to obtain a uniform bead suspension and transfer 10 $\mu L$ of the antibody slurry (see Note 4.6) into an Eppendorf tube
4. Wash the beads with 500 $\mu L$ of cold IP binding buffer by inverting the tube to resuspend the beads in the buffer, and then remove the buffer by placing the tube on a magnetic rack. Repeat for a total of four washes
...
9. Wash the beads with 400 $\mu L$ of cold IP binding buffer by inverting the tube to resuspend the beads. Collect the beads on a magnetic rack and discard the buffer. Repeat for a total of two washes
10. Wash the beads with 400 $\mu L$ of cold IP wash buffer by inverting the tube to resuspend the beads. Collect the beads on a magnetic rack and discard the buffer. Repeat for a total of two washes
11. Wash the beads with 400 $\mu L$ of cold water by inverting the tube to resuspend the beads. Collect the beads on a magnetic rack and discard the water. Repeat for a total of two washes
12. Add 50 $\mu L$ High Select Fe-NTA binding buffer and incubate beads for 10 min at room temperature on a shaker or Thermomixer. Ensure beads stay in suspension but are not splashed to the sides of the tube
...
16. Add 30 $\mu L$ elution buffer and collect elution into a fresh Eppendorf tube. Repeat elution once into the same collection tube
17. Dry peptides down to near-completeness, leaving ~5-10 $\mu L$ of liquid in the tube if continuing with data acquisition immediately. Otherwise, dry and store peptides at -80°C
18. Acidify with 40 $\mu L$ buffer A, check pH (should be at or below pH 4, see section 3.4.4)

### 3.3.5 LC-MS/MS analysis - Phosphoproteome
...
### 3.3.6 LC-MS/MS analysis - Total proteome

1.	Peptides are separated on an Aurora Elite XT column using the 40SPD Evosep method, interfaced with the mass spectrometer through an EASY-spray source
2.	On MS1 level, peptide masses were detected at a resolution of $120,000$, with an ion target of $3\times 10^{6}$, maximal injection time of $45 ms$, and RF lens set to $40\$\%$$
3.	MS2 spectra were recorded for the top 12 precursors with an intensity threshold of $10^3$, which were isolated at 0.7 Th and subjected to dynamic exclusion for 16 s. Normalized collision energy was set to $32\$\%$$ and spectra were recorded with a resolution of **30,000** with turboTMT set to TMTpro, ion target of **$1\times10^5$** and a maximal injection time of **Auto**.
