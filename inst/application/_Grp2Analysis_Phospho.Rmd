---
title: "Differential Expression Analysis for Phospho Peptides."
...
The phospho peptide identification and quantification were performed using: 
...
## Phospho Peptide identification
...
- Only phospho peptides are kept
...
The overall number of remaining phospho sites detected in this experiment is: `r grp2$RES$Summary$NrOfProteinsNoDecoys`.
Keep this?
...
Using Figure \@ref(fig:vennPeptides) we examine if we see the same phospho peptide in each group. We say a phospho peptide is unobserved in the group if it is absent in all samples and is present otherwise. A significant overlap among groups allows more precisely estimating the peptide abundance differences between the groups.
...
- site - unique site identifier protein_startphospho_endphosho_numPhos_numLocPhos_AApositionInProtein~PeptideSequence (small letter in peptide sequence indicate localized phosho sites)
...
## Differentially Expressed Phospho Sites

Here we use the FDR threshold of `r grp2$pop$FDRthreshold` and a difference threshold of `r grp2$pop$Diffthreshold` to select differentially expressed peptides. Table \@ref(tab:nrsignificant) summarizes the number of significant calls.
...
mycap <- paste0("Number of not significant and significant phospho sites."  )
...
(ref:SigPrey) Significant phospho sites obtained by applying the difference and FDR thresholds.
...
(ref:vennDiagramSig) Venn diagram showing the number of significant phospho sites for each contrast and their intersections.
...
- __raw_abundances__ table with empirical phospho site abundances.
- __normalized_abundances__ table with normalized phospho site abundances. 
- __raw_abundances_matrix__ A table where each column represents a sample and each row represents a phospho site and the cells store the empirical phospho site abundances.
- __normalized_abundances_matrix__ A table where each column represents a sample and each row represents a phospho site and the cells store the empirical phospho site abundances.
- __diff_exp_analysis__ A table with the results of the differential expression analysis. For each phospho site there is an row containing the estimated difference between the groups, the false discovery rate FDR, the 95% confidence interval, the posterior degrees of freedom. 
- __missing_information__ - spreadsheet containing information if a phospho site is present (1) or absent in a group (0).
- __phospho site_variances__ - spreadsheet which for each phospho site shows the variance (var) or standard deviation (sd) within a group, the number of samples (n) and the number of observations (not_na) as well as the group average intensity (mean).


The zip file also includes an "boxplot.pdf" file which shows the boxplots of normalized phospho site abundances for all the phospho sites in the dataset.


Jonas will write more here on additional downstream files
...
- diff (difference) - it is the difference of the phospho peptide abundance estimate of two groups
- FDR - false discovery rate
