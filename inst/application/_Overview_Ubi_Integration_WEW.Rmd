...
In this analysis, the enriched (GlyGly enrichment) as well as the pre-enriched datasets (total input) are first analyzed separately. After individual statistical analysis the two datasets are combined. For all ubiquitinated peptides where the protein is also quantified in the pre-enriched sample the protein statistics are linked to the ubiquitination peptide statistics. 
...
Next, we integrate the modified peptide statistics with the changes on protein level in the same way as described for MSstatsPTM (reference here). We refer to this as *MSstatsPTM adjustment*.
In brief, the difference in the levels of a ubi peptide (ubi site) between two conditions is normalized by the difference in the protein levels for that same comparison. By doing so, the difference of the proteins (if identified) are subtracted from the ubi peptides differences and the degrees of freedom as well as the T-statistics are further used to calculate the adjusted p-value for the adjusted difference.
