---
title: "Ubiquitination Differential Expression Analysis Overview and Integration"
...
In this analysis the enriched dataset (GlyGly enrichment) is analyzed first separately as well as the pre-enriched sample (total). After individual statistical analysis the two datasets are combined. For all ubiquitinilated peptides where the protein is also quantified and analyzed in the pre-enriched sample the protein statistics are added to the ubiquitination peptide statistics. 
...
When ubi peptide enrichment is done, usually also peptides are identified which are not identified in the proteome measurement.
...
In brief, the difference of the ubi peptide (ubi site) is adjusted by the difference of the protein change. By doing so, the difference of the proteins (if identified) are subtracted from the ubi peptides differences and the degrees of freedom as well as the T-statistics are further used to calculate the adjusted p-value for the adjusted difference.
