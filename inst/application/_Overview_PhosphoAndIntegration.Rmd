...
totS <- length(unique(data_diff_diff$originalSite))
totLocS <- length(unique(data_diff_diff$originalSite[data_diff_diff$AllLocalized]))
totSingS <- length(unique(data_diff_diff$originalSite[data_diff_diff$SinglePhosLocalized_bool]))
...
myd <- data_diff_diff |> select(originalSite, AllLocalized) |> distinct() 
...
barplot(table(data_diff_diff$AA[data_diff_diff$SinglePhosLocalized_bool]), main = "Phosphorylated AA residues")
```
...
p <- ggplot2::ggplot(na.omit(data_diff_diff[data_diff_diff$SinglePhosLocalized_bool,]), ggplot2::aes(x=AA, y=avgAbd.x)) + 
...
totProtFromSites <- data_diff_diff |> select(protein_Id, protein_Id.y) |> distinct() |> nrow()
totProtWGlobalProt <-  data_diff_diff |> select(protein_Id, protein_Id.y) |> distinct() |> filter(!is.na(protein_Id.y)) |> nrow()
totPepsWithGlobal <-  data_diff_diff |> select(protein_Id, originalSite, protein_Id.y) |> distinct() |> filter(!is.na(protein_Id.y)) |> nrow()
...
bb <- data_diff_diff |> dplyr::select(
  all_of(c("site",
           "originalSite",
           "protein_Id",
           "contrast",
           ...
           "MSstatsPTMadj_FDR")))
bb <- crosstalk::SharedData$new(bb, ~originalSite, group = "BB")
