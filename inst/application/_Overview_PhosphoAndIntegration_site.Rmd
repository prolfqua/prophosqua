...
# table of counts per modification
tbl_modAA <- table(data_diff_1C$modAA, useNA = "no")

# total unique sites
n_sites <- length(unique(data_diff_1C$site))

# if you want a combined object including the total:
tbl_combined <- c(tbl_modAA, Total = n_sites)

# convert to a data.frame for pretty printing
df_summary <- as.data.frame(tbl_combined, responseName = "count")
rownames(df_summary) <- NULL
colnames(df_summary) <- c("Modification", "Count")

# optionally use knitr::kable()
knitr::kable(df_summary, 
             caption = paste0("Overview on identified phospho sites (contrast = ", coi, ")"))
...
```{r tableAllProt, fig.cap="Differential PTM usage analysis results of all phospho sites (showing MSstatsPTM adjusted log2FCs and FDR)"}
bb <- data_diff_diff |>
  dplyr::select(all_of(c("site", "site","protein_Id","contrast","modelName.site","diff_diff","FDR_I")))
...
datax <- data_diff_diff |> dplyr::select(
  all_of(c("site", "contrast", "protein_Id",
           "diff.protein",
           "diff.site",
