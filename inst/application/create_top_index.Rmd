...
library(dplyr)
results <- prophosqua::read_ptm_h5mu(params$input_h5mu, prophosqua::PTM_results)
settings <- results$get_statistics()$get_inputs()$get_parameters()
...
link_if_exists <- function(file_path, link_path, label = NULL) {
  ...
  terminal <- grepl("\\.(xlsx|rds)$", file_path)
  if (file.exists(file_path) || terminal) {
...
generate_analysis_rows <- function(analysis_dir, relative_dir, analysis_type, ntoc_pdf_name) {
  ...
  if (!isTRUE(settings$run_kinase)) return(rows)
  ...
  if (isTRUE(settings$proptm3d$run)) {
    app <- link_if_exists(file.path(analysis_dir, "proptm3d/index.html"), file.path(relative_dir, "proptm3d/index.html"), "3D browser")
    rows <- c(rows, sprintf("| Protein structures | %s | |", app))
  }
  return(rows)
}

# Analysis directories
dpa_dir <- file.path(params$folder, analyses$dpa$subdir)
dpu_dir <- file.path(params$folder, analyses$dpu$subdir)
cf_dir <- file.path(params$folder, analyses$cf$subdir)
```

## Combined Results

The complete MuData contains the input experiments, PTM statistics, and enabled enrichment results. Excel and RDS delivery files are exported after all reports finish, with standardized columns and normalized abundances.

```{r combined_results, results='asis'}
cat("| Description | File |\n")
cat("|-------------|------|\n")
cat("| Complete analysis | [PTM_results.h5mu](PTM_results.h5mu) |\n")
...
rows <- generate_analysis_rows(dpa_dir, analyses$dpa$subdir, "DPA", "Site_differential_Expression_MultiContrast.pdf")
cat(paste0(rows, "\n", collapse = ""))

# Results xlsx
results_xlsx <- link_if_exists(file.path(dpa_dir, "Result_DPA.xlsx"), file.path(analyses$dpa$subdir, "Result_DPA.xlsx"), "xlsx")
...
dpu_overview <- link_if_exists(file.path(dpu_dir, "Result_DPU.html"), file.path(analyses$dpu$subdir, "Result_DPU.html"))
dpu_xlsx <- link_if_exists(file.path(dpu_dir, "Result_DPU.xlsx"), file.path(analyses$dpu$subdir, "Result_DPU.xlsx"), "xlsx")
cat(sprintf("| DPU Integration | %s | %s |\n", dpu_overview, dpu_xlsx))

main_html <- link_if_exists(file.path(dpa_dir, "Analysis_DPA_DPU.html"), file.path(analyses$dpa$subdir, "Analysis_DPA_DPU.html"))
if (nchar(main_html) > 0) {
  cat(sprintf("| DPA/DPU Analysis | %s | |\n", main_html))
}

# Generate standard rows using helper
rows <- generate_analysis_rows(dpu_dir, analyses$dpu$subdir, "DPU", "Site_differential_UsageChange_MultiContrast.pdf")
cat(paste0(rows, "\n", collapse = ""))
```
...
main_html <- link_if_exists(file.path(cf_dir, "Analysis_CorrectFirst_DEA.html"),
                            file.path(analyses$cf$subdir, "Analysis_CorrectFirst_DEA.html"))
main_xlsx <- link_if_exists(file.path(cf_dir, "CorrectFirst_PTM_usage_results.xlsx"),
                            file.path(analyses$cf$subdir, "CorrectFirst_PTM_usage_results.xlsx"), "xlsx")
cat(sprintf("| Main Analysis | %s | %s |\n", main_html, main_xlsx))

# Generate standard rows using helper
rows <- generate_analysis_rows(cf_dir, analyses$cf$subdir, "CF", "Site_differential_CorrectFirst_MultiContrast.pdf")
cat(paste0(rows, "\n", collapse = ""))
```
