...
For a paired experiment the difference is estimated by:
...
knitr::kable(grp2$RES$lfqData$factors(), caption = "LC-MS/MS samples annotation table. The content of the sampleName column is used as a short form to plot labels. The group to which a sample is assigned to is shown in the column group.")
...
The overall number of identified proteins with at least one ubi-site detected in this experiment is: `r grp2$RES$Summary$NrOfProteinsNoDecoys`.
...
To identify a high number of ubi-peptides an enrichment strategy was used. The ratio of ubi-peptides versus non-ubi peptides identified was:  __`r round(enrichment_ratio, 2)` percent__.
...
We also keep the decoy sequences because they allow us to re-estimate the proportion of falsely identified peptides in the list of differentially expressed peptides, which might differ from that of the entire dataset. The identifiers of the decoy peptides start with __REV__ or __rev__.
...
The absence of a peptide measurement in a sample might be biologically relevant or might point to technical problems. Significant differences in the set of peptides observed in the samples within a group typically indicate either technical problems or excessive biological variability. If one sample out of ten has a different set of peptides, it is likely an outlier and can be removed from the analysis. If the differences between the groups are significant but are small within the groups, this might systematically bias the difference estimates, i.e., produce false-positive or false-negative test results.
...
Using Figure \@ref(fig:vennPeptides) we examine if we see the same ubi-enriched peptide in each group. We say a ubi-enriched peptide is not observed in the group if it is absent in all samples but present otherwise. A significant overlap among groups allows more precisely estimating the peptide abundance differences between the groups.
...
Figure \@ref(fig:SDViolin), shows the coefficients of variations (CV) for all peptides computed on not normalized data. Ideally, the within group CV should be smaller than the CV of all samples. 
...
(ref:volcanoplot)  Volcano plot showing $-\log_{10}$ transformed FDR as a function of the difference between groups. The red line indicates the $-log_{10}(FDR)$ of FDR = `r grp2$pop$FDRthreshold`, while the green lines represent the the difference of minus and plus `r grp2$pop$Diffthreshold`. Orange dots differences and FDRs were estimated using missing value imputation.
...
cat("The table shown in Figure \\@ref(fig:SigPrey) lists all significant sites")
```
