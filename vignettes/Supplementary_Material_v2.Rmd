...
(ref:fig2usageQ64337) N-to-C usage plot showing differential usage of phosphorylation sites (vertical lines, color-coded by type of residue) along the protein sequences. The vertical lines are either dashed or solid, depending on the site's imputation status. The x-axis represents amino acid position from N to C terminus, while the y-axis shows $\log_2$ fold changes. Significantly regulated sites (FDR < 0.05 and FDR < 0.01) are highlighted with one or two red asterisks, respectively. The plot enables visualization of both overall protein regulation and site-specific phosphorylation changes in a single view.

```{r fig2usageQ64337, fig.cap="(ref:fig2usageQ64337)"}
x1 <- which(plot_data$protein_Id == "Q64337")
plot_data$plot[[x1]]
```

```{r fig2usageQ64337Table, include=FALSE}
dtab <- plot_data$data[[x1]] |> tidyr::separate(site, c("site",NA), sep="~")
knitr::kable(dtab) |> kableExtra::kable_styling(font_size = 9, latex_options = "scale_down")
```
...
```{r fig3densityCorrected, fig.cap="Density plot of the total proteome corrected PTM site abundances."}
...
```{r fig4FDRcorrfirst, fig.cap ="Volcano plot showing the -log10(FDR) as a function of the PTM site fold change."} 
...
We generate sequence logos for sites of significantly regulated serine and threonine phosphorylation. The plots are organized by contrast and regulation direction (up/down) to identify kinase-specific motifs (see Figure \@ref(fig:fig5seqlogoST).
...
```{r fig5seqlogoST, fig.width=9, fig.height=12, fig.cap="(ref:fig5seqlogoST)"}
