...
5. Interpreting DPA vs. DPU results for biological insights and generating publication-quality visualizations
...
Sample annotation is the foundation of any meaningful differential expression analysis, serving as the bridge between raw mass spectrometry files and biological interpretation. The annotation file maps each raw data file to its experimental conditions and further explanatory variables, e.g. Time. 
...
2. Here, we use `R` to extract experimental factors (e.g., Genotype, Timepoint) from the channel labels using the `tidyr::separate` funciton.
...
2. Differential expression analysis of the enriched proteomes for multi-site output: Multi-site analysis examines changes in modification patterns across multiple sites on the same protein. This is useful for understanding global modification changes. The `abundance_multi-site_None.tsv` file generated by `FragPipe` is used as input.
...
2. PTM Feature Usage for Protein `Q64337`: We display here the plot showing how the abundances of phosphorylation sites on protein `Q64337` changes between experimental conditions.
...
1. This analysis starts by loading the `prolfquapp` outputs, but this time we read the transformed and normalized total proteome protein abundances.
    ...
    tot_d <- ldata |> 
      dplyr::select(channel, UniprotID, normalized_protein_abundance)
...
2.  We are also loading normalized single-site abundances. In addition to loading the data we create a `prolfqua::LFQData` object using the `LFQData$new` constructor. The `LFQData` object provides many methods, to analyse and visualize quantitative proteomics data [@10.1021/acs.jproteome.2c00441].
    ...
    tot_res_dir <- prophosqua::dea_res_dir(project_dir, "singlesite_PTM", datedea)
...
4. Next we fit a linear models, which explaines the corrected PTM site abundances using the grouping `G_` of the samples we defined in the annotatation file. The model is fitted for each site using the `prolfqua::build_model` function.
