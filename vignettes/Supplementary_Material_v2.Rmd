...
We start by reading the sample names from the `FragPipe` output files using the `prolfqua_dataset.sh` script to create a dataset annotation file compatible with `prolfquapp`, that maps sample names to their experimental conditions.
...
We use `R` to extract experimental factors (e.g., Genotype, Timepoint) from the channel labels using the `tidyr::separate` function.
...
We start by defining the project name and analysis type. The `ptm_feature` parameter determines whether we analyze single phosphorylation sites or multi-site patterns. We run the single-site analysis, to run the multi-site analysis, change the `ptm_feature` parameter to `multisite`. 
...
The next code snipped exports $100$ `N-to-C` expression plots to a PDF file for further analysis and presentation (see also NOTE 4.8 why we limit the number of `N-to-C` plots written into a file).
...
We start by creating a report configuration which contains the project ID, order id and a workunit description to be displayed in the interactive HTML report.
...
Next we can render the interactive HTML report with comprehensive analysis results, including tables, plots, and statistical summaries. This report can be generated using the `prophosqua` package and for this analysis is available for download from Zenodo [@10.5281/zenodo.15830988].
...
To compute the contrasts we construct an array of contrasts from the annotation table which we constructed in Section 3.4.
...
We start by filtering the DPU data by $\|log_2(FC)\| > 0.58$ and $FDR \le 0.05$. The Table \@ref(tab:tableSignificantContrasts) summarizes the number of PTM-sites for each contrast, modification site, and up- or down-regulation. We observe that for serine (S) and threonine (T) there are many up and downregulated sites.
