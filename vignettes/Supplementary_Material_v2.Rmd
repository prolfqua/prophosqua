---
title: "Supplementary Material: Computational Protocol for Integration and Analysis of Total Proteome and PTM-Enriched Mass Spectrometry Data"
...
header-includes:
  - \usepackage{fancyhdr}
  - \pagestyle{fancy}
  - \fancyhead[CO,CE]{Supplementary Material - PTM Integration Protocol}
  - \fancyfoot[CO,CE]{\textbf{FGCZ} - www.fgcz.ch}
...
wd <- here::here("inst", "PTM_analysis_example")
...
# A. Differential expression analysis using `prolfquapp` for individual datasets
...
```{bash a2_get_scritps, results="hide"}
...
```{bash a3_create_dataset, results="hide"}
...
```{bash a4_runDEAtotal, echo = TRUE, results="hide"}
...
```{bash a5_multisite,eval=TRUE,results="hide"}
...
```{bash a5_singlesite,eval=TRUE, results="hide"}
...
datetoday <- "20250708"
project_dir <- "."#./inst/PTM_analysis_example/"
...
```{r b3_define_analysis_parameters}
fdr_threshold <- 0.01
...
```{r b6_plotQ64337, fig.cap="Q64337"}
...
```{r b9_generate_plots_usage, results='hide', message=FALSE}
if (ptm_feature == "multisite") {
...
```{r b8_show_usageQ64337, fig.cap="PTM feature usage for Q64337"}
x1 <- which(plot_data$protein_Id == "Q64337")
plot_data$plot[[x1]]
knitr::kable(plot_data$data[[x1]])
```
...
```{r b9_render_report_expression, message=FALSE}
...
```{r b9_renderdocument, eval = TRUE, results="hide"}
# Render HTML report
rmarkdown::render(
...
```{r b11_link_to_report, echo=FALSE, results='asis'}
...
significant_sites <- combined_test_diff |>
  filter(FDR_I < 0.05, abs(diff_diff) > log_2FC_threshold, !is.na(posInProtein), grepl("^KO_vs_WT", contrast)) |>
  dplyr::mutate(
...
```{r c1_makeseqlog, fig.width=6, fig.height=12, fig.cap=""}
