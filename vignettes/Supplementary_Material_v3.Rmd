...
Because generating N-to-C plots is computationally intensive, we generate plots only for proteins with at least one significant phosphorylation site or multi-site feature in any contrast. The False Discovery Rate (FDR) threshold for determining significance can be adjusted in the `n_to_c_expression_multicontrast` or `n_to_c_usage_multicontrast` functions, allowing researchers to balance computational efficiency with comprehensive visualization based on their specific considerations. When no significant PTM sites are detected despite biological expectations, one possible reason is insufficient statistical power, which can be mitigated by increasing the sample size (see Note \@ref(nosignificantPTM)).
...
if (ptm_feature == "multisite") {
  ...
  plot_data <- n_to_c_expression_multicontrast(
    combined_site_prot_long, FDR_threshold = 0.01,
...
if (ptm_feature == "multisite") {
  ...
  plot_data_usage <- n_to_c_usage_multicontrast(
    combined_test_diff_long, FDR_threshold = 0.05,
    max_plots = 100, include_proteins = c("Q64337"))
} else if (ptm_feature == "singlesite") {
  plot_data_usage <- n_to_c_usage_multicontrast(
