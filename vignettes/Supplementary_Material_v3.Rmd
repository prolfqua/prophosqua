...
Because generating N-to-C plots is computationally intensive, we generate plots only for proteins with at least one significant phosphorylation site or multi-site feature. The False Discovery Rate (FDR) threshold for determining significance can be adjusted in the `n_to_c_expression` or `n_to_c_usage` functions, allowing researchers to balance computational efficiency with comprehensive visualization based on their specific considerations. When no significant PTM sites are detected despite biological expectations, one possible reason is insufficient statistical power, which can be mitigated by increasing the sample size (see Note \@ref(nosignificantPTM)).
...
```{r abbreviations, echo=FALSE, results='asis'}
if (params$abbreviations) {
  cat("
## Abbreviations
