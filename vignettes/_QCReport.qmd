...
```{r themesetup, echo=FALSE, warning=FALSE, message=FALSE}
ggplot2::theme_set(ggplot2::theme_bw())
```

```{r a1getthedata}
if (params$psm == "" || params$fasta == "") {
...
x <- prophosqua::make_fasta_summary(fasta, as_string = TRUE)
...
config$opt_rt = "Retention"
config$opt_mz = "psmcharge"
...
config$hierarchy[["precursor"]] <- c("Modified.Peptide","Assigned.Modifications", "Charge")
config$hierarchy[["Spectrum"]] <- c("Spectrum")

config$factors[["experiment"]] <- "experiment"
...
allmods <- lfqdata$data_long() |> dplyr::select(mod_peptide_Id, Spectrum) |> dplyr::distinct()
...
xx <- lfqdata$data_long()
totalPerChannel <- xx |> dplyr::group_by(!!rlang::sym(lfqdata$get_config()$sample_name) ) |>
...
```{r relativeQuant, fig.cap = "Total abundance relative to the first channel."}
nudgeval <- 0.1
reference_sample <- totalPerChannel$sampleName[[1]]
reference_abundance <- totalPerChannel$totalAbundance[[1]]
totalPerChannel$relative_abundance <- totalPerChannel$totalAbundance / reference_abundance

totalPerChannel |> ggplot2::ggplot(ggplot2::aes(x = sampleName, y = relative_abundance)) + 
  ggplot2::geom_bar(stat = "identity", position = "dodge", colour = "black", fill = "white") + 
  ggplot2::geom_text(ggplot2::aes(label = round(.data$relative_abundance, 2)), nudge_y = nudgeval, angle = 65) +
  ggplot2::theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
# TODO add numbers to bars and table.

```

Normalization to the first channel (`r reference_sample`) helps identify systematic biases. Values should typically be within 0.5-2.0 fold of the reference.
