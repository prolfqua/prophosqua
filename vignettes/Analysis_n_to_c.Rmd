...
```{r loadData}
if (pipeline_mode) {
  message("Loading data from: ", params$input_h5mu, " (sheet: ", params$sheet, ")")
  data <- prophosqua::read_ptm_h5mu(params$input_h5mu, prophosqua::PTM_results)$get_tables()[[params$sheet]]
  output_dir <- if (!is.null(params$output_dir)) params$output_dir else dirname(params$input_h5mu)
} else {
...
data_info <- tibble(
  ...
  Value = c(
    if (pipeline_mode) basename(params$input_h5mu) else "Example data",
...
example_caps <- paste0(
  ...
  "contrast; each vertical stick with a head is one PTM site, coloured by modified residue ",
  "(S blue, T green, Y vermilion, not localized purple), dashed with an open head when the ",
  "value was imputed. Sites with FDR < 0.2 are labelled with residue and position, and the ",
  "asterisks give the site FDR (** < 0.05, * < 0.2); the yellow rectangle spans the protein from ",
