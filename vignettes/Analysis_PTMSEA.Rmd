...
```{r contrastPlots, results='asis', fig.height=8, fig.keep='all'}
...
```{r combinedDotplot, fig.width=14, fig.height=10}
...
```{r individualPlots, fig.width=14, fig.height=10, results='asis'}
...
```{r summaryHeatmap, fig.width=12, fig.height=10}
stopifnot(
...
```{r volcanoPlot, fig.width=12, fig.height=8}
...
```{r exportGseaPdf, results='hide', eval=pipeline_mode}
...
```{r exportGseaPdfVignette, results='hide', eval=!pipeline_mode}
...
Showing top 10 plots per contrast. `r if(pipeline_mode) paste0("**All ", n_gsea_plots, " plots exported to PDF.**")`

```{r gseaExamplePlots, results='asis', fig.width=10, fig.height=6}
for (ct in names(results)) {
  cat("\n\n## ", ct, " {.tabset}\n\n")
...
```{r exportVignette, eval=!pipeline_mode}
