...
```{r loadData}
if (pipeline_mode) {
  ptmsea <- prophosqua::ptm_enrichment_report_data(params$input_h5mu, "PTMSEA", params$analysis_type)
} else {
...
knitr::kable(
  ptmsea$data_info,
  caption = paste0(
    "Input of this PTM-SEA report: source data, analysis, the per-site statistic the sites ",
...
This report reads completed PTM-SEA results from `PTM_results.h5mu`. The terminal export step writes the following delivery files after all reports finish, including valid empty results.

```{r exportedFiles}
knitr::kable(
  tibble(
    Output = c("Excel results", "RDS object"),
    File = paste0("PTMSEA_", params$analysis_type, "_results", c(".xlsx", ".rds"))
  ),
