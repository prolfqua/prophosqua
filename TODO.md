...
Significant overlap between prophosqua vignettes and ptm-pipeline
templates:
...
- **prophosqua vignettes**: Use package example data, run full analysis,
  basic clusterProfiler plots
- **ptm-pipeline templates**: Use YAML params, read Excel/CSV, enhanced
  visualization via `enrichment_viz_utils.R`

## Recommended Solution

### 1. Move visualization functions to prophosqua

Create `R/enrichment_viz.R` with functions from
`ptm-pipeline/template/src/enrichment_viz_utils.R`:
...
Keep vignettes as minimal documentation: - Load example data - Run one
analysis - Show one plot using new viz functions
