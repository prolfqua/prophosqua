...
MEA <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(source, analysis, result = .compute_mea_stage(source, analysis)) {
      if (!inherits(source, "MotifEnrichment")) {
        stop("MEA requires MotifEnrichment")
      }
      ...
      .require_ptm_fields(result, c("mea_clean", "summary_df", "n_files", "has_results", "analysis_inputs"), "MEA")
